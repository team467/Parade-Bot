...
public class Drive extends SubsystemBase {
    ...
    private final PIDController feedback = new PIDController(KP, 0, KD);
    ...
    public void driveVelocity(double leftMetersPerSec, double rightMetersPerSec) {
        double leftvolts =
                feedforward.calculate(leftMetersPerSec)
                        + feedback.calculate(getLeftVelocityMetersPerSec(), leftMetersPerSec);
        double rightvolts =
                feedforward.calculate(rightMetersPerSec)
                        + feedback.calculate(getRightVelocityMetersPerSec(), rightMetersPerSec);
        io.setVoltage(leftvolts, rightvolts);
    }
